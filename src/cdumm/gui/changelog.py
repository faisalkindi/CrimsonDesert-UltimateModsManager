...
CHANGELOG = [
    {
        ...
        "date": "2026-05-11",
        "notes": [
            "<b>The Mods page now has an Uninstall Selected button next to New Folder, for the times you want to do a cleanup and start fresh.</b> The right-click menu has always carried an Uninstall N mods batch action that fires on whatever cards you Ctrl+Clicked, but the existence of that workflow was discoverable only by accident — there was no visible UI affordance saying \"yes, you can bulk uninstall\". The new button surfaces the same flow as a first-class action in the header row alongside New Folder and View Conflicts. It is greyed out when nothing is selected, lights up in red and shows a live count (\"Uninstall 5 selected\") as soon as you tick or highlight one or more cards, and always pops the standard confirmation dialog before doing anything destructive. Internally it reuses the existing batch-uninstall path so the disable → Apply (revert game files) → remove-from-DB flow is identical to the right-click menu, including the special-case fast path for mods that were already disabled (no Apply needed). Want to nuke your whole library? Click Select all, then this button. That doubles as the long-missing \"delete all mods\" answer for users prepping a clean slate.",
            "<b>The button reads both the enable-checkboxes AND the visual highlights as a UNION, never as a priority.</b> An earlier draft preferred the visual highlight (Ctrl+Click) when present, but that created a sticky-mode bug: plain-clicking a card body would silently capture the button onto the visual set and ignore subsequent checkbox toggles until you double-tapped Select all to clear the highlight. The shipped logic treats both inputs as a union — every ticked-or-highlighted mod feeds the button — so the count is always explainable as \"ticked OR highlighted\", checkbox toggles always update the count, and Ctrl+Click / Shift+Click on top of a checkbox set just adds more. The order of mods passed to the batch flow is preserved as load-order from <code>_mod_cards</code>, so the disable → Apply → remove pipeline still walks them in the same order the existing right-click batch path does.",
            "<b>Pressing Esc on the Mods page now clears any visual highlights, mirroring the muscle memory from Windows Explorer / VS Code / every other multi-select UI.</b> If you Ctrl+Click your way to a stray selection or want to back out before clicking Uninstall, Esc is the universal \"never mind\" key and it now works the way you expect. Only the transient highlight is cleared — the per-card enable-checkbox state is untouched on purpose, because that represents persistent game state and Esc is an \"abort selection\" key, not a \"revert state\" key. The shortcut is scoped to the Mods page's widget tree (<code>WidgetWithChildrenShortcut</code>), so it does not compete with the search box (which clears its own field on Esc), inline rename (which commits via <code>editingFinished</code>), or any other Esc handler in the rest of the window.",
            "<b>All three new strings are translated into every supported locale (16 languages).</b> The button label (no-count form), the count-aware form (\"Uninstall N selected\"), and the tooltip line that explains the dual ticked-or-highlighted selection model are now present in en, ar, de, es, fr, id, it, ja, ko, pl, pt-BR, ru, tr, uk, zh-CN, and zh-TW. Each language's verb matches the existing uninstall vocabulary already used elsewhere in that locale file (Spanish <code>Desinstalar</code>, German <code>deinstallieren</code>, Indonesian <code>Copot</code>, Japanese <code>アンインストール</code>, Korean <code>제거</code>, Polish <code>Odinstaluj</code>, Turkish <code>kaldır</code>, Ukrainian/Russian <code>Видалити/Удалить</code>, etc.), so the button reads consistently with the right-click menu's existing translated entry in the same locale. Languages with plural-genitive count forms (Polish, Russian) follow the same shape their existing <code>context_menu.uninstall_multi</code> strings already use. Native-speaker review of the tooltip wording is still welcome via Issue or PR.",
        ],
    },
    {
        "version": "3.3.2",
