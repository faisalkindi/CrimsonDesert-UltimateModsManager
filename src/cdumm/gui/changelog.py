...
CHANGELOG = [
    ...
    },
    {
        ...
        "notes": [
            ...
            "Thanks to AgentKush, who contributed five of the fixes in this release: the ReShade .addon64 install gate, folder-group preservation across updates, the preset picker height cap, the two static-analysis bug fixes, and the Python 3.10 zip-import guard.",
        ],
    },
    {
        "version": "3.3.23",
    ...
    },
    {
        ...
        "date": "2026-06-11",
        "notes": [
            "<b>Socket mods and other item mods that grow the item table now apply.</b> The apply step measured a change by its NEW size instead of the region it replaces, so any item-table change that made the table bigger (the Sockets modules of CD QOL Suite add about 2 KB) was always skipped with an 'exceeds file size' warning before it could even be compared. Found via falobos76's retest on GitHub (#191).",
            "<b>The item table's offset index is now rebuilt whenever the table grows or shrinks.</b> Without it, every record after the first size change had a stale index entry and the game read garbage. The same protection that storeinfo and equipslotinfo mods already had now covers item and skill mods (#191).",
            "<b>Mods editing different files inside the same small archive no longer fight each other.</b> Entry-level composition previously only ran for archives over 10 MB, so two mods touching the same small config archive (the materials/renderpass one both VAXIS mods use) were winner-takes-all even with zero file overlap. Reimport both mods on this version to pick up the fix. Found via RoGreat's report on GitHub (#199).",
            ...
            "<b>ALL item-table edits now go through the version-correct parser.</b> Simple field edits (cooldowns, tiers, categories) used to walk an outdated layout description on game version 1.10 and could silently do nothing or write to the wrong place. They now use the same parser that tracks the live game layout.",
            "<b>Mods that run scripts now ask for your permission first.</b> Dropping an archive containing a script used to execute it immediately with no prompt. CDUMM now explains what the script is and only runs it after you confirm.",
            "<b>Updating a mod by dropping a new version no longer deletes the old one first.</b> If the new import fails, your working version is restored instead of being lost.",
            "Startup cleanup no longer deletes mods that merely share a display name. Two variants like 'Trust Me 2x' and 'Trust Me 5x' look identical after name prettifying; removal now requires identical version and content.",
            "Cleanup of old overlay folders now happens only after an apply fully commits. A failed apply used to be able to roll back to game files that pointed at already-deleted folders, which crashed the game at boot.",
            "Mods applying on top of each other now adapt to the item table's real size when other mods already grew or shrank it, instead of reading a stale-length slice that cut the table mid-record (#191).",
            "Relocated patches that change size no longer overwrite the bytes right after them (grow) or leave stale bytes behind (shrink).",
            "Plugin INI files saved with a byte-order mark no longer lose their hook info in the ASI panel.",
            "Reapplying unchanged structure-changing mods no longer drops their index file from the overlay on the second apply.",
            "Test Mod mode can no longer delete your newest real mod when the trial import fails.",
            "The Nexus API key is no longer written into the log file.",
            "Renaming an ASI plugin works again (the rename used to silently revert).",
            "Switching language no longer paints every version pill green; unknown update state stays grey.",
            "Recovery flow no longer reports success when the apply never started, and no longer times out while a long rescan is still working.",
            "A worker process that fails to launch (e.g. blocked by antivirus) no longer leaves the app stuck on Busy until restart.",
            "Theme switching now restyles all tool pages, not just one.",
            "Find Culprit's progress log shows live progress lines again.",
            "Fixed the corrupted Chinese welcome message.",
            "Mods that target items by name instead of numeric key now apply on item and skill tables.",
            "The German translation is complete again (84 missing strings filled), and large parts of the settings, mods, and tools pages that were English-only are now translatable.",
    ...
    },
    {
        ...
        "date": "2026-06-10",
        "notes": [
            "<b>A mod you just updated no longer keeps asking to be updated.</b> After updating a mod through CDUMM, closing and reopening the app would flag the same mod as needing an update again, every time. The update was applied correctly, but the stored version number was left at the old value, so the next update check thought you were still on the old file. The version is now recorded with the update, so once a mod is current it stays current. Thanks to Faisal for the report.",
            "<b>Available updates no longer vanish from the mods list.</b> A check could find updates and show the red Click To Update pills, then a second check moments later made them disappear and report everything up-to-date. The second check skipped mods it had just verified (by design, to save API calls), but the result handling threw away what the first check had found about them. Each check now keeps the known state of mods it skipped, so updates stay visible until you actually install them. Thanks to GabrielNunesIT on GitHub (#194).",
            "<b>Item mods work again on the current game version.</b> Mods that edit item data (stack sizes, durability, cooldowns, buffs, sockets, and similar) were importing with zero changes on recent game versions, because the game updates changed the item table layout in four places and CDUMM was still reading the old one. The reader now matches the current game, verified byte-for-byte against the full item table, and a 1,820-change stack mod applies completely in testing. Thanks to IliyaBrook (#182) and falobos76 (#191) on GitHub.",
            "<b>Store mods that add items to a shop now apply.</b> Mods using the stock_data_list field (like adding pets to the Hernand pet store) were rejected because CDUMM could not write the store table's stock records. CDUMM now reads and rebuilds store stock lists natively, keeps existing store entries byte-identical, and refuses cleanly instead of guessing when a mod uses a part of the format that is not mapped yet. Thanks to pinapana on GitHub (#183).",
            "<b>Character Creator's equipment modules import now.</b> The Female Rapier and Shield module (and anything else editing equip slot hash lists) was rejected because CDUMM had no layout for the equip slot table. CDUMM now reads and rewrites those hash lists natively, leaving every other byte of the table untouched. Thanks to woowoots and lurkser on GitHub (#190).",
            "<b>A third launch option for Steam installs that refuse to start the game.</b> On some machines Steam answers both of CDUMM's launch methods with \"Game configuration unavailable\" even though pressing Play inside Steam works. Settings now offers Game exe (skip Steam), which starts CrimsonDesert.exe directly while Steam runs in the background, the same approach another manager uses successfully on an affected machine. The default behavior is unchanged. Thanks to lupo1190 on GitHub (#186).",
            "<b>VAXIS Water Physics Overhaul (and similar material mods) work on the current game again.</b> The latest game update moved those material files and started storing them encrypted. CDUMM already followed the move, but it was writing the mod's plain text where the game now expects encrypted data, so the materials silently failed to load. CDUMM now detects encrypted slots even when the file is not compressed and encrypts the replacement to match. Thanks to lupo1190 on GitHub (#199).",
        ],
    },
    {
        "version": "3.3.18",
    ...
    },
    {
        ...
        "date": "2026-06-02",
        "notes": [
            "<b>NexusMods downloads and the in-app update check no longer fail with CERTIFICATE_VERIFY_FAILED.</b> OmiCron07, linkinzelda, animalt68, HaZt-Panda and devCKVargas (GitHub #175 / #178 / #179 / #185) all hit the same error after the latest Crimson Desert game patch, with messages like 'certificate has expired (_ssl.c:1032)'. PyInstaller freezes whatever CA bundle ships with the Python build at exe-build time, so once any root in that bundle expires the frozen exe stops trusting half the internet, including api.nexusmods.com. CDUMM now builds its SSL contexts from certifi's Mozilla-tracked CA bundle (added as an explicit runtime dependency and shipped inside the exe), so the trust store stays current as long as certifi is updated. The three places this matters, Nexus API requests, GitHub release polling, and the NXM direct download, all go through the new helper now. Reporter and concept credit to OmiCron07 (PR #180).",
            "<b>The in-app update download path no longer hangs the UI on a different repro than v3.3.15 thought it fixed.</b> jikulopo (GitHub #170) ran v3.3.15 from source after the QueuedConnection fix landed, instrumented the download flow with extra log lines, and showed that the slot still ran on the worker thread and the InfoBar parent assignment still crashed with 'QObject::setParent: Cannot set parent, new parent is in a different thread'. Verified empirically against PySide6 6.11: connecting a worker signal to a free Python callable with QueuedConnection routes the call to the sender's thread, not the receiver's, because Qt has no QObject receiver to anchor the queue. The worker signals now go through a tiny QObject dispatcher parented to the main window, so the bound methods carry main-thread affinity and the slot really does run on main where InfoBar.success works. The mechanism is locked in by a regression test that pins both the old broken pattern and the fix pattern.",
            "<b>The Launch button writes useful diagnostics now.</b> lupo1190 (GitHub #186) reported the game stopped launching from CDUMM after their game install transitioned past a particular Steam state, but the cdumm.log contained zero entries from the click because every branch of _on_launch_game was silent. The handler now logs which channel it hands the launch off to (Steam URI with the resolved app_id, Xbox shell URI, or the direct bin64 exe) and the install-type detection result, so the next launch regression can be triaged from the log instead of asking the user to add instrumentation. Pure observability add, no behaviour change.",
            "<b>CDUMM respects the version in your mod's manifest instead of overwriting it with whatever the Nexus filename happens to encode.</b> Balzhur (GitHub #187): downloaded 'Easier QTE x2 v1.2.1' (Nexus 664) and CDUMM displayed it as v1.1. The mod author had bumped modinfo.json to 1.2.1 without renaming the Nexus file, so the archive filename still carried the 1-1 version slot, and the post-import block was running a filename parser whose output overwrote the manifest value the import worker had just written. The block now reads the existing mods.version row (which the import worker writes from modinfo.json or the JSON-mod top-level version BEFORE the post-import path runs) and only falls back to the filename or cached Nexus latest_version when the manifest path was empty or returned the import-handler default '1.0', or when the import is the click-to-update flow that lands an nxm_<mod>_<file>.bin temp with no manifest (the GitHub #164 case, still works).",
            "<b>Three Configure-window and click-target polish items from devCKVargas (GitHub #184).</b> One, the preset-picker dialog now scales with the parent window instead of capping at a 500 px sliver against empty space on 1440p. Floor stays at 500 px wide / 200 px tall for small windows, ceiling becomes max(500, 55 percent of parent width) and max(200, 45 percent of parent height) so big monitors get a usable picker. Two, the About-page link cards are clickable on the whole row now, not just the small Open button on the right, with a pointing-hand cursor on hover so the affordance is obvious. The Open button stays for discoverability. Three, double-clicking a mod card toggles its enabled state; the double-click programmatically toggles the existing checkbox so the same signal path runs as a manual click.",
            "<b>Settings has a new 'Steam launch method' dropdown.</b> lupo1190 (GitHub #186) hit a Steam-side regression where the URI launch path (steam://rungameid) fires 'Game configuration unavailable' on his 1.09 install while pressing Play in the Steam Client launches the game fine. The new Game-group setting in Settings exposes an opt-in 'Direct (-applaunch)' mode that runs steam.exe -applaunch <appid> instead of the URI, mirroring exactly what the Steam Client uses for its Play button. Default stays on URI so nothing changes for working installs. Affected users now have a one-click workaround they can flip without hand-editing the config table.",
            "<b>Clearer log line when the iteminfo / skill native parser fails on CD 1.09.</b> The raw struct.error that the parser threw before was hard to read in a bug-report bundle; the log now explicitly names GitHub #182 as the tracking issue for the 1.09 schema shift and points users at Format 2 offset-based byte patches as the working alternative until the Format 3 list-of-dict path catches up. Pure log-quality improvement, no behaviour change.",
        ],
    },
    {
        "version": "3.3.15",
        "date": "2026-05-29",
        "notes": [
            "<b>The in-app updater no longer freezes the GUI on download complete, and the new exe actually replaces the running one on Windows.</b> Elec0, jikulopo, devCKVargas and AwfulLon GitHub #170/#172 all reported the same symptom on different installs: the download bar fills, CDUMM3.exe.part appears, the bar disappears, and the UI hangs forever. There were two bugs feeding the same failure. First, the worker-thread signals connected to local Python callables defaulted to Qt's DirectConnection, so the slot that creates the success InfoBar ran inside the download thread; Qt refused to parent the InfoBar to the main window across thread boundaries and the GUI quietly froze (jikulopo identified this from a Python-source run: 'QObject::setParent: Cannot set parent, new parent is in a different thread'). The three slots (progress, done, failed) now connect with Qt.ConnectionType.QueuedConnection so they always run on the main thread. Second, when CDUMM3.exe lives at the same path the updater downloads to (Downloads folder, the default), os.replace fails with WinError 5 because Windows will not overwrite a running exe. The updater now uses the Chrome / Discord pattern: rename the live exe to CDUMM3.exe.old (Windows DOES allow that on a running binary), put the new exe in place, and clean up the .old file on the next launch. macOS and Linux are unaffected because their kernels allow overwriting a running binary outright.",
            "<b>Skip More Animations and similar Format 3 mods now actually import all of their paseq patches.</b> jikulopo GitHub #167: retested v3.3.14 and reported the same 18 patches skipped on a fresh apply with all the twin-retry code shipped to convert_json_patch_to_paz and process_json_patches_for_overlay. The bundle showed why: Format 3 JSON mods come through import_json_as_entr, which had no twin-retry at all. With the wrong twin picked at import time, all 6 changes per file failed the original-bytes check, the file was logged as skipped, and the mount-time retry that was supposed to catch this never got a chance because the import had already discarded the changes. import_json_as_entr now walks the same basename twins as the other two import paths and adopts whichever twin lands the most patches.",
            "<b>Socket-equipment mods exported from DMM now import on CDUMM.</b> pinapana GitHub #171: a new socket mod that targets the three iteminfo drop-default-data fields (_addSocketMaterialItemList, _socketValidCount, _useSocket) was rejected with 'no field_schema entry' and 'no list writer yet' messages, while neighbouring fields like equipable_hash applied cleanly. CDUMM already had the parser and writer support for all three (they live under drop_default_data inside the iteminfo native parser), but Format 3 field-name routing did not bridge DMM's binary-side underscored camelCase exports to CDUMM's dotted snake_case paths. The format3 parser now aliases the three names at parse time so both the validator and the iteminfo nested-path writer see drop_default_data.add_socket_material_item_list / .socket_valid_count / .use_socket and handle them with the existing nested-path machinery. No new RE was needed, the layout was already pinned by the 6235-record vanilla round-trip walk.",
            "<b>CDUMM has a native Linux build now, no Wine prefix required for the manager itself.</b> Andy Hunne contributed a Linux native path (PR #123): the manager runs as native Python on Linux while Crimson Desert itself keeps running under Proton/Steam. Includes Steam install auto-detection across the XDG default, the ~/.steam symlinks, Flatpak and Snap layouts, an nxm:// URL scheme handler via .desktop + xdg-mime, a placeholder ASI page (ASI plugins are intentionally not in scope because they need WINEDLLOVERRIDES on the Steam launch options, which the native build deliberately avoids), a launcher script that provisions the venv and optional Rust component, XDG portal file dialogs, and a Wayland app icon via cdumm.desktop + a hicolor theme entry. Tested end-to-end on Arch Linux with Python 3.14, and RoGreat helped validate the Nix-derivation packaging path. Windows users see no behaviour change, every new branch is gated behind IS_LINUX.",
        ],
    },
    {
        "version": "3.3.14",
    ...
    },
    {
        ...
        "date": "2026-05-26",
        "notes": [
            "<b>CDUMM relaunches cleanly after you quit it from the system tray.</b> umibozuT GitHub #162 reported that once they closed CDUMM (after launching the game through it, which hides the window to tray) they could not start CDUMM again until a full system restart. Root cause: the tray's Quit action calls close() on the main window, but Qt's quitOnLastWindowClosed only fires when a VISIBLE window is closed; the window had already been hidden by the launch-game flow, so the close did nothing for the event loop. The Qt event loop stayed running invisibly with the process still holding the single-instance .gui_lock, and every subsequent launch was rejected as 'another instance is already running'. closeEvent now explicitly calls QApplication.quit() so the event loop unwinds and atexit can release the lock file handle.",
            "<b>Mods that target a file whose archive holds two copies of it under the same internal path now apply against the correct copy.</b> AeGhBrA and jikulopo GitHub #167: Skip More Animations (Nexus 774) patches three paseq files (gimmick_craft_stone_repair_01.paseq and friends) where 0014/0.pamt stores the file twice with the same logical path but different blobs. CDUMM only kept the second-parsed entry and the apply loop silently skipped every change with a byte-mismatch. The PAMT index now records every twin under a sidecar key, and the JSON apply loop retries on the alternate when the first pick produces zero applied patches, adopting whichever twin actually carries the patch's expected original bytes. Same retry runs at mount-time, so JSON-only mods land correctly without needing to re-import. Pure additive code path: files with a unique entry behave exactly as before.",
            "<b>Reimport from source now refreshes the stored vanilla file sizes, so the Mod check warning clears after one reimport instead of sticking.</b> jikulopo GitHub #165 reported right-click then Reimport from source produced no visible effect: the mod stayed flagged with 'size changed (NNN -> NNN). Game updated, mod needs re-importing' and uninstall plus redownload from Nexus was the only thing that worked. Root cause: every site that persisted a per-mod vanilla file size used INSERT OR IGNORE, so the size was written once at the first import and never overwritten. After a Steam patch the new vanilla size never reached the row, the size-check kept firing against the old stored value, and reimport looked like a no-op even though it had run cleanly. All seven storage sites now use INSERT OR REPLACE so reimport refreshes the size in place.",
            "<b>The red 'Click to update' pill stays GREEN after a successful click-to-update download, instead of bouncing back to red on the next Nexus poll.</b> LordOfRhun GitHub #164: after updating a mod via the in-card pill, the pill turned green for a moment, Apply ran, and the next 15-minute Nexus poll re-flagged the same mod as outdated. Root cause: click-to-update downloads land as nxm_MODID_FILEID.bin temp files with no version embedded in the filename and no modinfo.json alongside, so the post-import version probe returned an empty string and the row's stored version stayed the OLD value. The fallback wrote that OLD value back, the next poll saw local less-than latest, and the pill went red again. The post-import path now consults the cached Nexus update entry (which already carries the latest_version that triggered the pill in the first place) when the filename probes come up empty, so the row stores the correct new version.",
        ],
    },
    {
        "version": "3.3.12",
